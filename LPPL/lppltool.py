...
szzs=pd.read_csv('szzs.csv')
SP = szzs['Price']
date=[parse(i) for i in szzs['Date']]
...
class Population:
    ...
    def SetFitness(self):
        ...
        self.prob=[i/np.sum(self.fitness) for i in self.fitness]
    ...
    def Fitness(self):
        ...
        for individual in list(self.populous):
            ...
            if ((state == False)):
                false += 1
                self.populous.remove(individual) #不收敛直接去除该个体
        self.SetFitness()
    ...
    def Eliminate(self):
        ...
        while (len(self.populous) > self.size * self.eliminate):
            A=np.random.choice(self.populous,p=self.prob)#轮盘赌算法
            self.populous.remove(A)
            self.SetFitness()
