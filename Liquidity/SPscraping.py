...
def simulation_alt(x,SPs,today_yield):
    ...
    DF_temp = SP_temp.merge(today_yield, how='inner', left_on='Date', right_on='Date')
    DF_temp['forecast']=cons+vol*DF_temp['vol']+curve*DF_temp['curve_spread']
    DF_temp['forecast_change']=DF_temp['forecast']-DF_temp['forecast'].shift(30)
...
def portfolio(IDs,notionals,df):
    ...
    print(np.sum(np.multiply(liquidity,durations[:,np.newaxis]),axis=1))
    liquidities = np.dot(ratio, liquidity).sort()

    max_liquidity = liquidities[-1]

    i = 1

    while max_liquidity >= 10:
        max_liquidity = liquidities[-1 - i]

        i += 1
    average_duration=np.dot(ratio,durations)
    return sum(notionals)*max_liquidity*average_duration*0.0001
...
def individual_liq_loss(x):
    ...
    notional=x['notional']
    if not notional:
        return 1000000*liquidity*ind_duration*0.0001
    if isinstance(notional,str):
        notional=notional.strip()
        if ',' in notional:
            notional=''.join(notional.split(','))
        notional=int(re.search('([0-9]+)|\w*([0-9]+)',notional).group(1))
    return notional*liquidity*ind_duration*0.0001
assets=pd.read_csv('E:\possible_asset.csv', encoding="ISO-8859-1")
...
mongostore=df.apply(lambda x:individual_liq_loss(x),axis=1).values
